...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("standalone");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		fOutput2_ptr = outputs[2];
		FAUSTFLOAT* fOutput2 = 0;
		...
		for (index = 0; (index <= (fullcount - 32)); index = (index + 32)) {
			...
			/* Compute code */
			for (int i = 0; (i < count); i = (i + 1)) {
				fZec196[i] = (fRec306[i] * ((4.5f * fRec307[i]) + 0.5f));
				
			}
			/* Recursive loop 653 */
			/* Compute code */
			for (int i = 0; (i < count); i = (i + 1)) {
				fZec197[i] = ((0.0500000007f * fRec308[i]) + 0.00100000005f);
				
			}
			/* Vectorizable loop 654 */
			/* Compute code */
			for (int i = 0; (i < count); i = (i + 1)) {
				fOutput0[i] = FAUSTFLOAT(((fZec195[i] + fZec196[i]) * fZec197[i]));
				
			}
			/* Vectorizable loop 655 */
			/* Compute code */
			for (int i = 0; (i < count); i = (i + 1)) {
				fOutput1[i] = FAUSTFLOAT((fZec195[i] * fZec197[i]));
				
			}
			/* Vectorizable loop 656 */
			/* Compute code */
			for (int i = 0; (i < count); i = (i + 1)) {
				fOutput2[i] = FAUSTFLOAT((fZec196[i] * fZec197[i]));
